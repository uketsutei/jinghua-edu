...
| 项目 | 建议 | 说明 |
| --- | --- | --- |
| 产品 | 轻量应用服务器（Lighthouse） | 别买「云服务器 CVM/ECS」，配置复杂且更贵 |
| 配置 | 2 核 2G（1 核 1G 也能跑） | 我们的后端很轻，2G 更稳 |
| 系统 | **Ubuntu 22.04 LTS** | 脚本按这个系统写的 |
| 地域 | 上海 / 广州 / 北京 | 选离学员近的 |
| 时长 | 1 年 | 新用户通常有折扣 |
...
**第 1 步：放行端口**
进云控制台 →「防火墙」（有的叫安全组）→ 添加规则：
协议 **TCP**，端口 **3000**，来源 `0.0.0.0/0`，允许。

**第 2 步：上传项目**
把整个 `菁华教育网课平台` 文件夹传到服务器（用 scp、宝塔面板、或者云控制台的
网页文件管理都行）。落在 `/root/jinghua-edu` 这类目录即可。
...
脚本会自动完成：装 Node（走国内镜像，1 分钟内）→ 复制项目到 `/opt/jinghua-edu`
→ 配置**开机自启**（systemd，进程崩了会自动拉起）→ 放行本机防火墙 → 自检并打印网址。

完成后访问 `http://服务器IP:3000/` 即可。常用管理命令：

```
systemctl status jinghua-edu      # 看状态
systemctl restart jinghua-edu     # 重启
tail -f /var/log/jinghua-edu.log  # 看日志
```

### 关于域名和备案

- **用 IP 访问不需要备案**，今天就能给学员用
- 想用 `jinghua-edu.com` 这种域名，大陆服务器必须先做 **ICP 备案**（免费，7～20 天），
  备案通过后再配 Nginx + HTTPS 就能用上漂亮网址
- 不想等备案，可以换成**香港/新加坡**的轻量服务器，免备案、能立刻配域名和 HTTPS，
  但国内访问速度一般
